...
# custom_fuc = {"everything_mode", "generate_ masks", "postprocess_small_regions", \
...
#                 "prompt_encoder", "mask_decoder", "postprocess_masks", "image_encoder", "preprocess"}

# custom_fuc = {"everything_mode", "filter_by_predicted_IoU", "calculate_stability_score", "threshold_masks_and_calculate_boxes", \
#                 "filter_boxes_that_touch_crop_boundaries", "compress_to_RLE", \
#                 "prompt_encoder", "mask_decoder", "postprocess_masks", "image_encoder", "preprocess"}

custom_fuc  = {"predict_masks", "Select the correct mask", "Concatenate output tokens", \
    "Expand per-image data in batch direction to be per-mask", "Run the transformer", "Upscale mask embeddings" \
    "predict masks using the mask tokens", "Generate mask quality predictions"}
...
def main(args: argparse.Namespace) -> None:
    print("Loading model...")
    if args.model_type == "tinysam":
        sam = sam_model_registry[args.model_type](encoder_ckpt=args.encoder, decoder_ckpt=args.decoder)
    else:
    ...
    for t in targets:
        ...
        # 打印表格
        table = custom_results_event_list.table(sort_by="cuda_time_total")
        print(table)
